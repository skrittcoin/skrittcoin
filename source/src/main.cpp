...
#if defined(NDEBUG)
# error "SkrittCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xbe1ac4ca1b0d490157e7d45608e3936b5b29b559d3a15fe01dba256ba9a4b237");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // SkrittCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "SkrittCoin Signed Message:\n";
...
{
    // SkrittCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 725 * COIN;

            if(nHeight == 1)
            {
            nSubsidy = 307576 * COIN;
            }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // SkrittCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // SkrittCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1522610438, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Fox News Apr 1st 2018 North Korean dictator K pop stars perform";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 725 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04380e58ce861d12b1e9acc0479ba5a4f4b10ef08cea44bae36cb0f0d668977c25f67e4a77909d25bd4e3814dedffe76aff68f90043322bbe92e2d8ad80d1482c4") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1522610438;
        ...
        assert(block.hashMerkleRoot == uint256("0xb7af374e2de85a2aab34e255d9e525f46bdddf8a4c07d0ac0bdc06c54eb08b6d"));
...
unsigned char pchMessageStart[4] = { 0x7f, 0xda, 0xd2, 0xfd }; // SkrittCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SkrittCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SkrittCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SkrittCoinMiner(CWallet *pwallet)
{
    printf("SkrittCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("skrittcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SkrittCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SkrittCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SkrittCoinMiner, pwallet));
}
